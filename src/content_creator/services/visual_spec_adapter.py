...
def project_to_visual_spec(project: VideoProject, stage: Region | None = None) -> VisualSpec:
    ...
    for index, item in enumerate(project.timeline):
        ...
        if index:
            ...
            transitions.append(TransitionSpec(id=f"transition-{index - 1}-{index}", from_scene=f"scene-{index - 1}", to_scene=f"scene-{index}", start_frame=item.start_frame - duration, duration_frames=duration, preset=TransitionPreset.flash_zoom_blur, params={"incoming_scale": 1.14, "blur_px": 24, "flash_peak": 0.95}))
    duration_frames = max((scene.start_frame + scene.duration_frames for scene in scenes), default=1)
    return VisualSpec(composition=CompositionSpec(width=project.width, height=project.height, fps=project.fps, duration_frames=duration_frames), layout=LayoutSpec(preset=LayoutPreset.center_stage, regions={"stage": stage}), scenes=scenes, transitions=[expand_transition_preset(transition, scenes[index + 1].layers[0].id) for index, transition in enumerate(transitions)])
