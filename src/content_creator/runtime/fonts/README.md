...
All fonts in this directory are project-local browser assets. The Animation Agent may reference only these whitelisted files through relative `runtime/fonts/...` URLs.
...
The font files are committed with the project so Chromium output does not depend on fonts installed on macOS, CI, or Linux render hosts.
