...
def create_layout_plan(items: list[tuple[SceneNarrative, ImageSemanticProfile | None]], global_style: str = "editorial", *, context: dict | None = None, preferences: dict | None = None, feedback_reason: str = "", avoid_fonts: set[str] | None = None) -> tuple[LayoutPlan, dict]:
    ...
    prompt = json.dumps({
        "task": "Design subtitle typography for a mobile video. Return only JSON LayoutPlan. Select concrete font_id values only from font_registry. Keep every media block exactly x=0,y=430,width=1080,height=610,fit=contain. You may design text placement, hierarchy, alignment, role, weight and immutable content variant. When copy_density_intent is increase, every frozen content_id is required and should form clear upper/lower hierarchy outside the media stage. Use at most two font IDs across the video. No CSS, font paths, new copy, media changes or transition changes.",
    ...
    try:
        ...
        retry_prompt = json.dumps({
            "task": "Return a corrected LayoutPlan JSON object using this valid baseline. Keep all keys and immutable content references, including every text block when copy_density_intent is increase. You may only change text_blocks geometry, alignment, typography_role, font_id, style_intent, weight, max_lines and variant_id/content_hash pairs already present in the narrative. Keep media_blocks unchanged. Use at most two registered font IDs.",
