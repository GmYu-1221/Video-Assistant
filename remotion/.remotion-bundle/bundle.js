...
const ImageFrame = ({ src, motion }) => {
  const frame = (0,esm/* useCurrentFrame */.UC)();
  const { durationInFrames } = (0,esm/* useVideoConfig */.Bk)();
  const p = (0,esm/* interpolate */.GW)(frame, [0, durationInFrames], [0, 1], { extrapolateRight: "clamp", easing: esm/* Easing */.GS.inOut(esm/* Easing */.GS.quad) });
  let scale = 1.04, x = 0, y = 0;
  if (motion === "zoom_in" || motion === "ken_burns") scale = 1.04 + 0.1 * p;
  if (motion === "zoom_out") scale = 1.14 - 0.1 * p;
  if (motion === "pan_left") x = -4 * p;
  if (motion === "pan_right") x = 4 * p;
  if (motion === "pan_up") y = -4 * p;
  if (motion === "pan_down") y = 4 * p;
  return /* @__PURE__ */ (0,jsx_runtime.jsx)("img", { src, style: { width: "100%", height: "100%", objectFit: "cover", transform: `translate(${x}%,${y}%) scale(${scale})` } });
};
...
const TransitionFactory = (transition, key) => {
  ...
  switch (transition.type) {
    case "slide":
      return /* @__PURE__ */ (0,jsx_runtime.jsx)(TransitionSeries.Transition, { timing, presentation: slide_slide({ direction: transition.direction === "from-left" ? "from-left" : "from-right" }) }, key);
    case "wipe":
      return /* @__PURE__ */ (0,jsx_runtime.jsx)(TransitionSeries.Transition, { timing, presentation: wipe({ direction: transition.direction === "from-right" ? "from-right" : "from-left" }) }, key);
    case "flip":
      return /* @__PURE__ */ (0,jsx_runtime.jsx)(TransitionSeries.Transition, { timing, presentation: flip({ direction: transition.direction === "from-left" ? "from-left" : "from-right" }) }, key);
    case "zoom_blur":
      return /* @__PURE__ */ (0,jsx_runtime.jsx)(TransitionSeries.Transition, { timing, presentation: zoomBlur({ rotation: 0.15 }) }, key);
    case "fade":
    default:
      return /* @__PURE__ */ (0,jsx_runtime.jsx)(TransitionSeries.Transition, { timing, presentation: fade() }, key);
  }
};
...
const Slideshow = (props) => {
  ...
  return /* @__PURE__ */ (0,jsx_runtime.jsxs)(esm/* AbsoluteFill */.H1, { style: { backgroundColor: "black" }, children: [
    /* @__PURE__ */ (0,jsx_runtime.jsx)(TransitionSeries, { children: props.timeline.flatMap((item, index) => {
      ...
      const sequence = /* @__PURE__ */ (0,jsx_runtime.jsx)(TransitionSeries.Sequence, { durationInFrames: item.duration_frames + transitionFrames, children: /* @__PURE__ */ (0,jsx_runtime.jsx)(ImageFrame, { src: `${base}/${(asset == null ? void 0 : asset.relative_path) ?? ""}`, motion: (asset == null ? void 0 : asset.motion) ?? "ken_burns" }) }, `sequence-${item.asset_id}`);
