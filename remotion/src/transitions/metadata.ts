...
export type TransitionMetadata = {implementation: TransitionImplementation; complexity: number; defaultDurationFrames: number; supportedDirections: string[]; usesMask: boolean; usesTransform: boolean; allowDistortion?: boolean};
...
export const TransitionMetadataRegistry: Record<string, TransitionMetadata> = {
  ...
  crossfade: custom(.2,8), black_flash: custom(.8,4), white_flash: custom(.9,3), push: custom(.5,6,false,true), whip: {...custom(.8,5,false,true), allowDistortion:true}, stretch_whip: {...custom(.9,6,false,true), allowDistortion:true}, digital_wipe: custom(.6,6,true), iris: custom(.5,8,true), clock_wipe: custom(.6,8,true), blinds: custom(.6,8,true), pixel_reveal: custom(.7,6), glitch: {...custom(.9,5,false,true), allowDistortion:true}, light_leak: custom(.7,5),
};
