...
import {TransitionEffectRenderer, type TransitionEffectPlan} from '../transitions/TransitionEffectRenderer';
...
export const VisualEffectRenderer = (event: VisualEvent, key: string, children?: React.ReactNode): ReactElement => {
  if (event.phase === 'transition') {
    if (event.type !== 'template_transition') {
      throw new Error(`Unknown visual transition effect: ${event.type}`);
    }
    return TransitionEffectRenderer({from_asset_id: event.source_asset_id ?? '', to_asset_id: event.target_asset_id ?? '', type: event.type, duration_frames: event.duration_frames, params: event.params as TransitionEffectPlan['params']}, key);
  }
