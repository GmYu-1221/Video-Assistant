...
def test_director_creative_intent_reaches_remotion_animation_and_render_data(tmp_path, monkeypatch):
    ...
    plan = DirectorPlan.model_validate({"timeline": [{
        ...
        "creative_intent": {
            "scene_id": "image-001",
            "description": "Image appears through particle assembly and rotates into view",
            "movement": "back-to-front rotation",
            "emotion": "cinematic",
            "style": "cinematic", "energy": 0.8,
            "effects": ["particle dissolve", "motion blur"],
        },
    }]})
    director_item = plan.timeline[0]
    assert director_item.creative_intent is not None
    assert "particle_flip_reveal" not in director_item.creative_intent.description
    ...
    storyboard = plan_to_storyboard(plan, "cinematic")
    ...
    rendered = compile_render_plan(project, storyboard)
    payload = json.loads(Path(rendered.output.render_data).read_text(encoding="utf-8"))
