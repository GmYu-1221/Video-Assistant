...
def test_multiple_segments_are_retimed_without_exceeding_three_point_five_seconds():
    bundle, profiles = _bundle(3, 120, density=.9)
    url_video._retime_resolved_bundle(bundle, profiles, bpm=90, fps=30)
    assert all(2.5 <= item.duration_frames / 30 <= 3.5 for item in bundle.resolved)
