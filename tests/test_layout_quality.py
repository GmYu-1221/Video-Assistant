...
def test_validator_rejects_text_over_known_subject_and_undeclared_collision():
    ...
    assert "persistent_title_collision" not in codes
...
def test_opening_uses_template_independent_media_safe_copy():
    ...
    assert layout.media_blocks[0].bbox.model_dump() == {"x": 0, "y": 655, "width": 1080, "height": 610}
    assert layout.media_blocks[0].fit == "contain"
    assert layout.text_blocks
    assert all(block.bbox.y + block.bbox.height <= 1860 for block in layout.text_blocks)
    assert all(block.caption_style_intent == CaptionStyleIntent.explanatory for block in layout.text_blocks)
