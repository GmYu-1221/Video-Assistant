...
def test_mock_chat_reports_llm_unavailable_without_mutating_plan(tmp_path, monkeypatch):
    ...
    assert updated.current_plan.timeline[1].duration_frames == 90
    assert "LLM 当前不可用" in _
    assert "Scene 01" in format_plan(updated)
    assert "Animation Design:" in format_plan(updated)
